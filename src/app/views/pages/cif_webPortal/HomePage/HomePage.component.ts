...
import { ChangeDetectorRef, Component, ElementRef, HostListener, Inject, NgZone, OnInit, ViewChild } from '@angular/core';
...
export class HomePageComponent implements OnInit {
  ...
  // ✅ Single flag: true once ALL images are preloaded and ready to display
  ...
  constructor(
    ...
    // ✅ FIX #1: Inject NgZone so we can bring zone-unaware callbacks back into Angular's zone
    private ngZone: NgZone,
    @Inject(DOCUMENT) document: Document,
    private router: Router,
    private route: ActivatedRoute) { }

  // ✅ FIX #2: Replaced new Image() (zone-unaware) with fetch() (zone-patched by zone.js).
  //    fetch() is automatically intercepted by zone.js, so Angular change detection fires
  //    when Promise.all resolves — no NgZone.run() needed, no loading spinner needed.
  //    'force-cache' reuses a cached response if available; the blob() call ensures the
  //    full image bytes are downloaded and stored in the browser cache before we reveal
  //    the grid, so <img> tags render instantly when the *ngIf flips to true.
  private fetchAndCacheImages(imageUrls: string[]): Promise<void> {
    const validUrls = imageUrls.filter(url => !!url);
    const promises = validUrls.map(url =>
      fetch(url, { cache: 'force-cache' })
        .then(res => res.blob())   // consume body → browser caches the image bytes
        .catch(() => null)         // skip broken URLs silently; don't block the reveal
    );
    return Promise.all(promises).then(() => {
      this.areFacilityImagesLoaded = true;  // zone.js fires change detection here automatically
    });
  }
  ...
  getAllInstruments(): void {
    ...
    this.CIFwebService.GetAllInstrumentsData().subscribe({
      next: response => {
        ...
        // ✅ FIX: Use fetch()-based preloader — zone-aware, no spinner needed
        const imageUrls = this.tmpsInstrumentsDataData.map(item => item.imageUrl);
        this.fetchAndCacheImages(imageUrls).then(() => {
